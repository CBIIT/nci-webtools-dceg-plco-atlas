...
export const TreeSelect2 = forwardRef(
  (
    ...
    const expandParents = displayTreeParent => {
      ...
      parents.map(item => {
        if (root.getElementsByClassName('collapse-button-text-' + item.id)[0]) {
          if (
            root
              .getElementsByClassName('collapse-button-text-' + item.id)[0]
    ...
    const checkAllChildrenDisabled = item => {
      ...
      const itemImportDates = itemAllLeafs.map(obj =>
        obj ? obj.import_date : undefined
      );
    ...
    const checkParents = item => {
      ...
      if (!singleSelect) {
        ...
        const checkAllLeafsSelectedResult = checkAllChildrenLeafsSelected(
          itemAllLeafs
            .filter(obj => (alwaysEnabled ? obj : obj.import_date))
            .map(obj => obj.id),
    ...
    const handleSelect = item => {
      ...
      if (singleSelect) {
        ...
        let values = [...value];
        let newValues = getAllLeafs(item).filter(obj =>
          alwaysEnabled ? obj : obj.import_date
        );
        if (containsAllVals(values, newValues)) {
          ...
          for (let i = 0; i < newValues.length; i++) {
            if (
              root.getElementsByClassName('leaf-checkbox-' + newValues[i].id)[0]
            ) {
              root.getElementsByClassName(
          ...
          if (
            ...
            root.getElementsByClassName('children-of-' + item.id)[0].style
              .display &&
            root.getElementsByClassName('children-of-' + item.id)[0].style
              .display === 'none'
          ) {
              ...
              <li
                ...
                <div className="d-flex align-items-center">
                  <button
                    ...
                    <i
                      className={
                        'fa fa-plus-square collapse-button-text-' + item.id
                      }></i>
                  </button>
                  ...
                  <input
                    ...
                    style={{
                      ...
                      cursor:
                        singleSelect ||
                        (!singleSelect && checkAllChildrenDisabled(item))
                          ? 'not-allowed'
                          : 'pointer'
                    }}
                  ...
                  <button
                    title={
                      singleSelect
                        ? 'Show/hide ' + item.title + ' phenotypes'
                        : 'Select/deselect all ' + item.title + ' phenotypes'
                    }
                    className="ml-1"
                    style={{
                      ...
                      whiteSpace: 'nowrap',
                      overflow: 'hidden',
                      color: checkAllChildrenDisabled(item)
                        ? '#D3D3D3'
                        : 'unset'
                    }}
                    disabled={
            ...
            return (
              ...
              <li
                ...
                <input
                  title={
                    singleSelect
                      ? 'Select ' + item.title + ' phenotype'
                      : 'Select/deselect ' + item.title + ' phenotype'
                  }
                  style={{
                    cursor:
                      alwaysEnabled || item.import_date
                        ? 'pointer'
                        : 'not-allowed'
                  }}
                ...
                <button
                  ...
                  style={{
                    ...
                    width: '65%',
                    color:
                      alwaysEnabled || item.import_date ? 'unset' : '#D3D3D3'
                  }}
                  disabled={alwaysEnabled || item.import_date ? false : true}
        ...
        return dataAlphabeticalFiltered.map(item => (
          <div
            ...
            <input
              ...
              style={{
                cursor:
                  alwaysEnabled || item.import_date ? 'pointer' : 'not-allowed'
              }}
              ...
              checked={
                (singleSelect && value && value.id === item.id) ||
                (!singleSelect && value.map(item => item.id).includes(item.id))
              }
              disabled={alwaysEnabled || item.import_date ? false : true}
              onChange={e => handleSelect(item)}
            />
    ...
    const checkAllLeafsSelected = () => {
      ...
      data.tree.map(item =>
        allLeafs.push(
          getAllLeafs(item).filter(obj =>
            alwaysEnabled ? obj : obj && obj.import_date
          )
        )
      );
          ...
          <div className="bg-secondary border-bottom d-flex align-items-center py-1">
            {listType === 'categorical' && (
              <>
                <button
                  title={
                    expandAll ? 'Hide all phenotypes' : 'Show all phenotypes'
                  }
                  style={{ all: 'unset' }}
                  ...
                  {expandAll && (
                    <i
                      ...
                      style={{ cursor: !data ? 'not-allowed' : 'pointer' }}></i>
                  )}
                  {!expandAll && (
                    <i
            ...
            <input
              ...
              style={{
                cursor: singleSelect || !data ? 'not-allowed' : 'pointer'
              }}
          ...
          {data && (
            <>
              <ul
                ...
                style={{
                  display:
                    data && listType === 'categorical' ? 'block' : 'none',
                  listStyleType: 'none',
                  textOverflow: 'ellipsis',
                  overflowY: 'auto',
                  overflowX: 'hidden',
              ...
              <ul
                ...
                style={{
                  display:
                    data && listType === 'alphabetical' ? 'block' : 'none',
                  listStyleType: 'none',
                  textOverflow: 'ellipsis',
                  overflowY: 'auto',
                  overflowX: 'hidden',
