                    ...
                    <div className="row left py-1"> 
                        ...
                        <div className="col-md-auto ml-1 px-0" style={{maxHeight: '300px', overflow: 'auto'}}>
                            { collapseCriteria && (
                                <>
                                    <span>
                                        {searchCriteriaVariantLookup && searchCriteriaVariantLookup.phenotypes ? searchCriteriaVariantLookup.phenotypes[0] : 'None'}
                                    </span>
                                    <span className="ml-1">
                                        {
                                            searchCriteriaVariantLookup && searchCriteriaVariantLookup.phenotypes && searchCriteriaVariantLookup.phenotypes.length > 1 ? (<span>and</span>) : (<></>)
                                        }
                                        <button 
                                            ...
                                            aria-expanded={!collapseCriteria}>
                                            {
                                                searchCriteriaVariantLookup && searchCriteriaVariantLookup.phenotypes && searchCriteriaVariantLookup.phenotypes.length > 1 ? searchCriteriaVariantLookup.phenotypes.length - 1 + ` other${searchCriteriaVariantLookup.phenotypes.length - 1 === 1 ? '' : 's'}` : ''
                                            }
                                        </button>
                                    </span>
                                </>
                            )}
                            { !collapseCriteria && searchCriteriaVariantLookup && searchCriteriaVariantLookup.phenotypes &&
                        ...
                        <div className="col-md-auto ml-1 px-0">
                            ...
                            {searchCriteriaVariantLookup && searchCriteriaVariantLookup.variant ? searchCriteriaVariantLookup.variant : 'None'}
                            
                            <span className="mx-3">|</span>
                            
                            <span><b>Gender</b>: </span>
                            {searchCriteriaVariantLookup && searchCriteriaVariantLookup.gender ? displayGender(searchCriteriaVariantLookup.gender) : 'None'}
                        </div>
                    </div>
                    
                    <div className="right py-1">
                        <span>Total Phenotypes: </span>
                        {searchCriteriaVariantLookup && searchCriteriaVariantLookup.totalPhenotypes ? searchCriteriaVariantLookup.totalPhenotypes : 'None'}
                    </div>
