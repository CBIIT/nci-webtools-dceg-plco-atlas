...
export function PhenotypesTabs(props) {
  ...
  const setSelectedPlot = async selectedPlot => {
    ...
    if (
      !phenotypeData[selectedPlot] ||
      !Object.keys(phenotypeData[selectedPlot]).length
    ) {
      dispatch(
        updateBrowsePhenotypesPlots({
          loading: true
    ...
    return (
      <Table
        ...
        pagination={paginationFactory({
          showTotal: phenotypeData.frequency
            ? phenotypeData.frequency.length > 0
            : false,
  ...
  const DistributionTable = ({
    ...
    if (
      phenotypeData.type === 'categorical' ||
      phenotypeData.type === 'continuous'
    ) {
      columns = [
      ...
      let values =
        phenotypeData.distributionCategories || phenotypeData.categories;
    ...
    <div style={{ minHeight: '600px' }}>
      ...
      <Tabs
        ...
        <Tab
          ...
          phenotypeData.frequency.length ? (
            <>
        ...
        {[
          phenotypeData.ageName && {
            key: 'frequencyByAge',
            title: 'Frequency By Age'
          },
          { key: 'frequencyBySex', title: 'Frequency By Sex' },
          {
            key: 'frequencyByAncestry',
            title: 'Frequency By Self-Reported Race'
          }
        ]
          .filter(Boolean)
          .map(t => (
            <Tab
              ...
              {!loading &&
              phenotypeData[t.key] &&
              phenotypeData[t.key].counts &&
              Object.keys(phenotypeData[t.key].counts).length ? (
                <>
                  ...
                  {phenotypeData && phenotypeData[t.key] && (
                    <>
                      {displayType[t.key] === 'plot' && (
                        <div className="text-center">
                          {(/continuous/.test(phenotypeData.type) ||
                            (/binary/.test(phenotypeData.type) &&
                              t.key === 'frequencyByAge')
                            ? GroupedAreaChart
                            : BarChart)({
                            data: phenotypeData[t.key][frequencyType[t.key]],
                            categoryPrefix: t.key.match(
                              /frequencyBy(Age|Sex|Ancestry)/
                            )[1],
                            type: phenotypeData.type,
                            categories:
                              phenotypeData.type === 'binary'
        ...
        <Tab
          ...
          {!loading &&
            ...
            phenotypeData &&
            phenotypeData.relatedPhenotypes && (
              <PhenotypesRelated
