...
describe('Actions Module', function () {
    ...
    test('updateSummaryResults() generates expected action', () => {
        const data = {test: 1};
        expect(updateSummaryResults(data)).toEqual({type: UPDATE_SUMMARY_RESULTS, data})
    });

    test('updateManhattanPlot() generates expected action', () => {
        const data = {test: 1};
        expect(updateManhattanPlot(data)).toEqual({type: UPDATE_MANHATTAN_PLOT, data})
    });
    ...
    test('updateSummaryTable() generates expected action', () => {
        const data = {test: 1};
        const key = 'example';
        expect(updateSummaryTable(key, data)).toEqual({type: UPDATE_SUMMARY_TABLE, key, data})
    });

    test('setSummaryTableLoading() generates expected action', () => {
        const data = true;
        expect(setSummaryTableLoading(data)).toEqual({type: UPDATE_SUMMARY_TABLE, key: 'loading', data})
    });

    test('updateSummarySnp() generates expected action', () => {
        const data = {test: 1};
        const key = 'example';
        expect(updateSummarySnp(key, data)).toEqual({type: UPDATE_SUMMARY_SNP, key, data})
    });

    test('updateSummarySnpTable() generates expected action', () => {
        const data = {test: 1};
        const key = 'example';
        expect(updateSummarySnpTable(key, data)).toEqual({type: UPDATE_SUMMARY_SNP_TABLE, key, data})
    });

    test('setSummarySnpLoading() generates expected action', () => {
        const data = true;
        expect(setSummarySnpLoading(data)).toEqual({type: UPDATE_SUMMARY_SNP, key: 'loading', data})
    });

    test('updateVariantLookup() generates expected action', () => {
        const data = {test: 1};
        expect(updateVariantLookup(data)).toEqual({type: UPDATE_VARIANT_LOOKUP, data})
    });

    test('updateVariantLookupTable() generates expected action', () => {
        const data = {test: 1};
        expect(updateVariantLookupTable(data)).toEqual({type: UPDATE_VARIANT_LOOKUP_TABLE, data})
    });

    test('updatePhenotypeCorrelations() generates expected action', () => {
        const data = {test: 1};
        expect(updatePhenotypeCorrelations(data)).toEqual({type: UPDATE_PHENOTYPE_CORRELATIONS, data})
    });
    ...
    test('updateBrowsePhenotypes() generates expected action', () => {
        const data = {test: 1};
        expect(updateBrowsePhenotypes(data)).toEqual({type: UPDATE_BROWSE_PHENOTYPES, data})
    });

    test('updateBrowsePhenotypesPlots() generates expected action', () => {
        const data = {test: 1};
        expect(updateBrowsePhenotypesPlots(data)).toEqual({type: UPDATE_BROWSE_PHENOTYPES_PLOTS, data})
    });
