...
const {
	file,
	create_partitions_only: createPartitionsOnly,
	host,
	port,
	db_name,
	user,
...
async function importPhenotypes() {
	...
	let records = parse(readFile(inputFilePath), {
		...
		on_record: (record, context) => {
			// ensure that:
			// id and parent_id study_id are numeric values or null
			...
			for (let key in record) {
				...
				if (/^NULL$/i.test(value) || !value.length) value = null;
				record[key] = value;
			}
			...
			else record.parent_id = null;

			if (record.study_id !== undefined && record.study_id !== null)
				record.study_id = +record.study_id;
			else record.study_id = null;

			if (record.type === 'ordinal') record.type = 'categorical';
	...
	while (records.length) {
		...
		let hasOrderedParent = orderedRecords.find(
			(e) => e.id === record.parent_id
		);
		...
		if (record.parent_id === null || hasOrderedParent)
			...
			orderedRecords.push(record);
		} else console.log('ERROR: Could not find parent for', record);
	}
	...
	for (let record of orderedRecords) {
		...
		//console.log(!createPartitionsOnly, record);
		if (!createPartitionsOnly)
			await connection.execute(
				`INSERT INTO phenotype (id, parent_id, display_name, name, description, type, age_name, sex_specific, study_id)
                    VALUES (:id, :parent_id, :display_name, :name, :description, :type, :age_name, :sex_specific, :study_id)`,
				record
			);
		...
		for (let table of [aggregateTable, pointTable]) {
			...
			if (!partitionRows.length) {
				console.log(
					`[${duration()} s] Adding partition ${partition} to ${table}...`
				);
				await connection.query(
