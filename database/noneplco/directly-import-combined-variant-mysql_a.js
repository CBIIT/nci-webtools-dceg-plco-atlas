...
allfiles.forEach((line) => {
	...
	async function exportVariants({
		...
		try {
			...
			const sexes = distinct(stratifiedColumns.map((c) => c.sex)).filter(
				Boolean
			);
                    ...
                    `);
						...
						const getQQPoints = (numPoints, maxValue) =>
							new Array(numPoints)
								...
								.map((_, i) => i + 1)
								.map((x) =>
									Math.floor(((x * maxValue ** 0.5) / numPoints) ** 2)
								)
								.reduce(
									(acc, curr) =>
