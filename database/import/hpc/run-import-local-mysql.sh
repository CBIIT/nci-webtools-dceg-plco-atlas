...
if [ -z "$1" ]
    then
        ...
        echo "USAGE: sh run-import-local-mysql.sh <DB_USER> <DB_PASS> <BASE_DIR> <COMMANDS_FILE> <NUM_JOBS>"
...
sbatch --gres=lscratch:200 --mem=512g --cpus-per-task=36 --partition=largemem --time=08:00:00 --wrap="sh import-local-mysql.sh $DB_USER $DB_PASS $BASE_DIR $COMMANDS_FILE $NUM_JOBS"
