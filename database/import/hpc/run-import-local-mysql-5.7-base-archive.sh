...
if [ -z "$1" ]
    then
        ...
        echo "USAGE: sh run-import-local-mysql-base-archive.sh <DB_USER> <DB_PASS>"
        echo "EXAMPLE: sh run-import-local-mysql-base-archive.sh sample_username sample_password"
        exit 1
fi
DB_USER=$1
