...
function getValidColumns(tableName, columns) {
	...
	return columns.length ? intersection(columns, validColumns) : validColumns;
}
/**
...
function deferUntilConnected(config, logger) {
	...
	async function tryConnection() {
		try {
			return await mysql.createConnection(config).promise().query('select 1');
		} catch (e) {
...
async function getSummary(
	{ connection, logger },
	{ phenotype_id, table, sex, ancestry, p_value_nlog_min, limit, offset, raw }
) {
	// validate parameters
	phenotype_id = parseInt(phenotype_id);
	if (
		!phenotype_id ||
		!(await hasRecord(connection, 'phenotype', { id: phenotype_id }))
	)
	...
	if (
		!ancestry ||
		!(await hasRecord(connection, 'lookup_ancestry', { value: ancestry }))
	)
...
async function getVariants({ connection, logger }, params) {
	...
	if (
		...
		throw new Error('A valid ancestry must be provided');
	// validate chromosome
	if (params.chromosome === 'X' || params.chromosome === 'x')
		params.chromosome = '23';
	if (
		params.chromosome &&
		!(await hasRecord(connection, 'chromosome_range', {
			id: +params.chromosome,
		}))
	)
		throw new Error('A valid chromosome must be provided');
	// validate/sanitize snps
	if (params.snp) {
		params.snp = params.snp
			.match(/[\w:]+/g)
			.filter((x) => /^[a-z0-9\:]+$/gi.test(x));
	}
	if (params.order_by) {
				...
				.map((s) =>
					/^rs\d+$/i.test(s)
						...
						: `(chromosome = "${s
								...
								.replace(/chr/gi, '')
								.replace(/X/gi, '23')
								.toUpperCase()}" AND position = ${s.split(':')[1]})`
				)
				.join(' OR ')}`
		),
	...
	const order = ['asc', 'desc'].includes(params.order) ? params.order : 'asc';
...
async function exportVariants({ connection, logger }, params) {
	...
	if (!phenotypes.length)
		throw new Error('Valid phenotype ids must be provided');
	const { data, columns } = await getVariants(
		{ connection, logger },
		{
			...params,
			columns: [
	...
	return {
		filename:
			[
				...
				params.p_value_min &&
					params.p_value_max &&
					`p_value_${params.p_value_min}_${params.p_value_max}`,
				params.p_value_nlog_min &&
					params.p_value_nlog_max &&
					`p_value_${10 ** -params.p_value_nlog_max}_${
						10 ** -params.p_value_nlog_min
					}`,
				params.position_min &&
					params.position_max &&
					`mb_range_${params.position_min / 1e6}_${params.position_max / 1e6}`,
			]
				.filter(Boolean)
				.join('_') + '.csv',
...
async function getPoints(
	{ connection, logger },
	{ phenotype_id, sex, ancestry, raw, limit, offset }
) {
...
async function getMetadata({ connection, logger }, params) {
	...
	const phenotype_id = params.phenotype_id
	...
	const conditions = [
		coalesce(
			params.phenotype_id,
			`m.phenotype_id IN (${getPlaceholders(phenotype_id.length)})`
		),
		coalesce(params.sex, `sex IN (${getPlaceholders(sex.length)})`),
		coalesce(
			params.ancestry,
			`ancestry IN (${getPlaceholders(ancestry.length)})`
		),
		coalesce(
			params.chromosome,
...
async function getGenes({ connection, logger }, params) {
	if (
		!params.transcription_start ||
		!params.transcription_end ||
		!params.chromosome
	) {
		throw new Error(
			'Chromosome, transcription_start, and transcription_end are required'
		);
	}
	// todo: only use numeric values for genes
	if (+params.chromosome === 23) params.chromosome = 'X';
	let sql = `
        SELECT 
            id, 
            name, 
...
async function getPhenotypes({ connection, logger }, params = {}) {
	let columns = getValidColumns('phenotype', params.columns)
		.map(quote)
		.join(',');
	if (params.q) {
		columns = [
...
async function getParticipants({ connection, logger }, params) {
	...
	let sql = `
        select
            ${[
							columns.includes('phenotype_id')
								? `pd.phenotype_id as phenotype_id`
								: ``,
							columns.includes('value')
								? `round(pd.value, :precision) as value`
								: ``,
							...
							columns.includes('genetic_ancestry')
        ...
        group by ${columns
					.map((c) => (c === 'value' ? `round(pd.value, :precision)` : c))
					.join(',')}
...
async function getPhenotypeParticipants({ connection, logger }, params) {
	...
	if (!type)
		throw new Error(
			'Please specify a valid type:  "frequency"|"frequencyByAge"|"frequencyBySex"|"frequencyByAncestry"|"related"'
		);
	if (!id) throw new Error('A phenotype id must be provided');
	...
	phenotype.categoryTypes = {
		frequencyByAge: ['55-59', '60-64', '65-69', '70-74', '75-79'],
		frequencyBySex: ['female', 'male'],
		frequencyByAncestry: [
			'white',
			'black',
			'hispanic',
	...
	if (phenotype.type === 'binary') {
		phenotype.categories = [
			`Without ${phenotype.displayName}`,
			`With ${phenotype.displayName}`,
		];
		...
		if (type === 'frequency') {
			...
			phenotype.frequency = (
				await connection.execute(binaryFrequencySql, { id })
			...
			const keyValueArrayReducer =
				(key, value) =>
				(obj = {}, curr) => ({
					...obj,
					[curr[key]]: [...(obj[curr[key]] || []), +curr[value]],
				});
			let keyAlias = key === 'age' && phenotype.ageName ? `pd.age` : `p.${key}`;
			let selectParticipants = `SELECT ${keyAlias} AS \`key\`
                    FROM participant_data pd
            ...
            `;
			logger.debug(
				`getPhenotype binary ${key} distribution sql: ${binaryDistributionSql}`
			);
			// determine the distribution of unique values for the specified key
			const [distribution] = await connection.execute(binaryDistributionSql, {
				id,
			});
			phenotype[type] = {
				counts: distribution.reduce(keyValueArrayReducer('key', 'counts'), {}),
				percentage: distribution.reduce(
					keyValueArrayReducer('key', 'percentage'),
					{}
				),
			};
		}
		...
		if (phenotype.type === 'categorical') {
            ...
            `;
			logger.debug(
				`getPhenotype categorical categories sql: ${categoricalCategoriesSql}`
			);
			let [categoryRows] = await connection.execute(categoricalCategoriesSql, {
				id: params.id,
			});
			...
			keyGroupReducer = (categories, distributionKey) => (acc, curr) => {
				let categoryName = categoryRows.find(
					(c) => c.value == curr.group
				).label;
				if (!phenotype.distributionCategories.includes(categoryName))
					return acc;
				if (!acc[categoryName])
					acc[categoryName] = new Array(categories.length).fill(0);
				acc[categoryName][categories.indexOf(curr.key)] =
					+curr[distributionKey];
				return acc;
			};
			...
			keyGroupReducer = (categories, distributionKey) => (acc, curr) => {
				if (!acc[curr.group])
					acc[curr.group] = new Array(categories.length).fill(0);
		...
		if (type === 'frequency' && phenotype.type === 'categorical') {
            ...
            `;
			logger.debug(
				`getPhenotype categorical frequency sql: ${categoricalFrequencySql}`
			);
			phenotype.frequency = (
				await connection.execute(categoricalFrequencySql, { id: params.id })
			)[0].map((e) => e.count);
            ...
            `;
			logger.debug(
				`getPhenotype continuous frequency sql: ${continuousFrequencySql}`
			);
			let [frequencyRows] = await connection.query(continuousFrequencySql, {
				id: params.id,
			});
			...
			logger.debug(
				`getPhenotype categorical ${key} distribution sql: ${distributionSql}`
			);
			const [distributionRows] = await connection.execute(distributionSql, {
				id: params.id,
			});
			phenotype[type] = {
				counts: distributionRows.reduce(
					keyGroupReducer(categories, 'counts'),
					{}
				),
				percentage: distributionRows.reduce(
					keyGroupReducer(categories, 'percentage'),
					{}
				),
			};
		}
	...
	for (let key in phenotype.categoryTypes) {
		phenotype.categoryTypes[key] =
			phenotype.categoryTypes[key].map(asTitleCase);
	}
			...
			for (let key of ['counts', 'percentage']) {
				if (phenotype[type][key]) {
					for (let subkey of Object.keys(phenotype[type][key])) {
						...
						phenotype[type][key][asTitleCase(subkey)] =
							phenotype[type][key][subkey];
...
function getConfig(key) {
	if (!key) throw new Error('A valid configuration key must be provided');
	const allowedKeys = ['downloadRoot', 'exportRowLimit'];
	return allowedKeys.includes(key) ? { [key]: config[key] } : null;
}
async function getDownloadLink({ connection, logger }, { phenotype_id }) {
	...
	let [downloadLinkRows] = await connection.execute(sql, { phenotype_id });
...
async function getShareLink({ connection, logger }, { share_id }) {
	if (!share_id) throw new Error('A valid share_id must be provided');
	let sql = `
	...
	let [shareLinkRows] = await connection.execute(sql, { share_id });
...
async function setShareLink({ connection, logger }, { route, parameters }) {
	...
	let shareLinkRows = await connection.execute(sql2, { id: results.insertId });
	return pluck(shareLinkRows);
}
async function getPrincipalComponentAnalysis(
	{ connection, logger },
	{ phenotype_id, platform, pc_x, pc_y, raw, limit }
) {
	// validate phenotype id
	if (
		!phenotype_id ||
	...
	if (!platforms.includes(platform))
		throw new Error(
			`A valid platform must be provided. Supported platforms include: ${platforms.join(
				', '
			)}`
		);
	if (!pc_x || !pc_y || isNaN(pc_x) || isNaN(pc_y))
		throw new Error(
			'Valid  principal components must be provided as pc_x and pc_y values'
		);
	if (pc_x == pc_y) throw new Error('Principal components must be different');
